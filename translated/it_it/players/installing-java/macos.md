...
Questa guida ti spiegherà come installare Java 25 su macOS.
...
Prendi nota del numero della versione: nell'esempio sopra è `25.0.9`.
...
Consigliamo di usare la [build di OpenJDK 25 da Adoptium](https://adoptium.net/temurin/releases?version=25&os=mac&arch=any&mode=filter):

![Pagina da cui si scarica Temurin Java](/assets/players/installing-java/macos-download-java.png)

Assicurati di aver selezionato la versione "25 - LTS", e scegli il formato d'installazione `.PKG`.
...
Se hai già installato [Homebrew](https://brew.sh), puoi alternativamente usare `brew` per installare Java 25:
...
Quando l'installazione è stata completata, puoi verificare che Java 25 sia attivo aprendo di nuovo il Terminale e scrivendo `$(/usr/libexec/java_home -v 25)/bin/java --version`.
