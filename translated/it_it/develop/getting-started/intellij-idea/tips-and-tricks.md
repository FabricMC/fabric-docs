---
title: Consigli e Trucchetti per IntelliJ IDEA
...
o clicca il <kbd>Pulsante Centrale del Mouse</kbd> (la rotellina) sul nome. Così eviterai lunghe sessioni di scorrimento o una ricerca manuale della definizione che troveresti in un altro file.
...
[Breakpoints](../../debugging#breakpoints) verranno inoltre mostrati qui.
...
Si può accedere velocemente al menu Generate con <kbd>Alt</kbd>+<kbd>Ins</kbd> (<kbd>⌘ Comando</kbd>+<kbd>N</kbd> su macOS) o andando su `Code` in cima e selezionando `Generate`.
In un file Java, potrai generare costruttori, getter, setter, fare override o implementare metodi, e molto altro.
Puoi generare accessori ed invocatori se hai installato [Minecraft Development plugin](./setting-up#installing-idea-plugins)
...
## Visualizzare il Bytecode {#viewing-bytecode}

Visualizzare il bytecode è necessario se si scrivono mixin. Puoi vedere il bytecode di una classe proveniente da una libreria (per esempio le classi di Minecraft) aprendo la classe nell'editor, per poi selezionare `Show Bytecode` nel menu `View`.

![Pulsante "Show bytecode" nel menu "View"](/assets/develop/getting-started/intellij/show_bytecode.png)

![Bytecode della classe java/lang/String.class](/assets/develop/getting-started/intellij/bytecode_example.png)
...
Anton Arhipov di Jetbrains ha anche tenuto un discorso approfondito riguardo a Corrispondenza Regex, Completamento di Codice, Debugging e molti altri argomenti legati a IntelliJ.
