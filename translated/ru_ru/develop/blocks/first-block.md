...
Чтобы добавить перевод, необходимо создать ключ перевода в файле перевода - `assets/<0>/lang/en_us.json`.
...
Вы можете перезапустить игру или создать свой мод и нажать <0>F3</0> + <0>T</0>, чтобы применить изменения. После этого вы увидите, что у блока есть имя в творческом инвентаре и других местах, например на экране статистики.

## Модели и текстуры {#models-and-textures}

Все текстуры блоков можно найти в папке `assets/<0>/textures/block` - пример текстуры для блока «Condensed Dirt» доступен для бесплатного использования.
...
- `assets/<0>/models/item/condensed_dirt.json`
...
Перезапустите игру или перезагрузите только assets с помощью <0>F3</0> + <0>T</0>, чтобы применить изменения. Вы должны будете увидеть текстуру блока в инвентаре и в мире:
...
При разрушении блока в режиме выживания вы можете увидеть, что блок не выпадает. Возможно для этого вам понадобится эта функция, однако, чтобы блок выпадал как предмет при разрушении, необходимо реализовать его таблицу добычи. Файл таблицы добычи должен быть помещен в папку `data/<0>/loot_table/blocks/`.
