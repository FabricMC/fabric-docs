...
Jede Zeile in der Gruppe gibt einen Codec, einen Attributname und eine Getter-Methode an. Der Aufruf `Codec#fieldOf` wird verwendet, um den Codec in einen [Map Codec](#mapcodec) zu konvertieren, und der Aufruf `forGetter` gibt die Getter-Methode an, die verwendet wird, um den Wert des Feldes von einer Instanz der Klasse abzurufen. In der Zwischenzeit gibt der Aufruf `apply` den Konstruktor an, der zur Erstellung neuer Instanzen verwendet wird. Beachte, dass die Reihenfolge der Felder in der Gruppe die gleiche sein sollte wie die Reihenfolge der Argumente im Konstruktor.
...
Der Aufruf von `Codec#fieldOf` wird einen `Codec<T>` in einen `MapCodec<T>` umwandeln, der eine Variante, aber keine direkte Implementierung von `Codec<T>` ist. MapCodecs, wie der Name schon sagt, werden garantiert in eine Key-to-Value-Map oder ihr Äquivalent in den verwendeten DynamicOps serialisiert. Einige Funktionen können einen solchen Codec über einen normalen Codec erfordern.

Diese besondere Art, einen `MapCodec` zu erstellen, verpackt im Wesentlichen den Wert des Quellcodecs in eine Map, mit dem angegebenen Feldnamen als Schlüssel. Zum Beispiel würde ein `Codec<BlockPos>`, wenn er in JSON serialisiert wird, wie folgt aussehen:
...
**Seit 1.20.2**, Minecraft selbst (nicht DFU!) bietet jedoch Codecs#createStrictOptionalFieldCodec\`, das die Deserialisierung fehlschlagen lässt, wenn der Feldwert ungültig ist.
...
`Codec.intRange` und seine Kumpels ‚`Codec.floatRange` und `Codec.doubleRange` können verwendet werden, um einen Codec zu erstellen, der nur
Zahlenwerte innerhalb eines bestimmten **inklusiven** Bereichs akzeptiert. Dies gilt sowohl für die Serialisierung als auch für die Deserialisierung.
...
Codec.pair" fasst zwei Codecs, `Codec<A>` und `Codec<B>`, zu einem `Codec<Pair<A, B>` zusammen. Beachte, dass dies nur mit Codecs funktioniert, die in ein bestimmtes Feld serialisiert werden, wie z. B. [konvertierte `MapCodec`s](#mapcodec) oder [Record Codecs](#merging-codecs-for-record-like-classes).
...
Für die Verarbeitung von Maps mit beliebigen Schlüsseln, wie zum Beispiel `HashMap`s, kann `Codec.unboundedMap` verwendet werden. Dies gibt einen `Codec<Map<K, V>>` für einen gegebenen `Codec<K>` und `Codec<V>` zurück. Der resultierende Codec wird in ein JSON-Objekt serialisiert, oder was auch immer für die aktuellen dynamischen Ops verfügbar ist.
...
Wie du sehen kannst, funktioniert dies, weil `Identifier.CODEC` direkt zu einem String-Wert serialisiert wird. Ein ähnlicher Effekt kann für einfache Objekte, die nicht in Strings serialisiert werden, erzielt werden, indem man sie mit [xmap & friends](#mutually-convertible-types-and-you) konvertiert.
...
Codec<BlockPos> blockPosCodec = Vec3d.CODEC.xmap(
    // Konvertiert ein Vec3d Instanz zu einer BlockPos Instanz
    vec -> new BlockPos(vec.x, vec.y, vec.z),
    // Konvertiert eine BlockPos Instanz zu einer Vec3d Instanz
    pos -> new Vec3d(pos.getX(), pos.getY(), pos.getZ())
);

// Wenn du eine existiertende Klasse (beispielsweise `X`) über diesen Weg
// zu deiner eigenen Klasse (`Y`) konvertierst, ist es schön die Methoden
// `toX` und eine statische Methode `fromX` zu `Y` hinzuzufügen und
// Methoden Referenzen in deinem `xmap` Aufruf zu nutzen.
```

#### flatComapMap, comapFlatMap und flatXMap

`Codec#flatComapMap`, `Codec#comapFlatMap` und `flatXMap` sind ähnlich wie xmap, erlauben aber, dass eine oder beide der
Konvertierungsfunktionen ein DataResult zurückgeben. Dies ist in der Praxis nützlich, da eine bestimmte Objektinstanz möglicherweise nicht nicht immer für die Konvertierung gültig ist.

Nimm zum Beispiel Vanille `Identifier` her. Während alle Bezeichner in Strings umgewandelt werden können, sind nicht alle Strings gültige Bezeichner, Daher würde die Verwendung von xmap zu hässlichen Ausnahmen führen, wenn die Umwandlung fehlschlägt.
...
Diese Methoden sind zwar sehr hilfreich, aber ihre Bezeichnungen sind etwas verwirrend, daher findest du hier eine Tabelle, damit du dir merken kannst, welche Methode du verwenden musst:
...
- Ein `Codec<BeanType<?>>`, der auf dieser Registry basiert. Wenn du eine `net.minecraft.registry.Registry` verwendest, kannst du sie mit `Registry#getCodec` leicht erstellen.
...
// Und darauf aufbauend, ist hier unser Registry Dispatch Codec für Bohnen! 
