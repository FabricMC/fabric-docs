...
## Підготовка авдіофайлу {#preparing-the-audio-file}

Ми збираємося створити новий **зациклений** звук для іншого `SoundEvent`. Якщо ви можете знайти авдіофайл, який уже безперешкодно відтворюється, просто виконайте дії, описані в розділі [створення власних звуків](../sounds/custom). Якщо звук ще не зациклюється ідеально, нам доведеться підготувати його до цього.

Знову ж таки, більшість сучасних DAW (Digital Audio Workstation Software) мають бути здатні на це, але я люблю використовувати [Reaper](https://www.reaper.fm/), якщо редагування авдіо є дещо складнішим.
...
![Reaper завантажується з авдіофайлом](/assets/develop/sounds/dynamic-sounds/step_0.png)

Ми можемо почути та побачити, що механізм запускається на початку та зупиняється в кінці, що не дуже добре для циклічних звуків. Виріжмо їх і налаштуємо маркери вибору часу відповідно до нової довжини. Також увімкніть режим `Toggle Repeat`, щоб авдіо відтворювалося циклічно, поки ми його налаштовуємо.

![Шаблонний авдіофайл](/assets/develop/sounds/dynamic-sounds/step_1.png)
...
Це постійний звук, який зберігає свою частоту протягом усього авдіо. Тому простого фільтра еквалайзера має бути достатньо, щоб відфільтрувати його.
...
Якщо ми відтворимо авдіо зараз, утримуючи вікно фільтра еквалайзера відкритим, фільтр еквалайзера покаже вхідний авдіо на своєму дисплеї.
Ми бачимо там багато нерівностей.

![Вияснення проблеми](/assets/develop/sounds/dynamic-sounds/step_3.png)

Якщо ви не є навченим авдіоінженером, ця частина здебільшого стосується експериментів і методу проб і помилок. Між вузлом 2 і 3 є досить сильна нерівність. Перемістімо вузли так, щоб знизити частоту лише для цієї частини.

![Знижено погану частоту](/assets/develop/sounds/dynamic-sounds/step_4.png)

Крім того, за допомогою простого фільтра еквалайзера можна досягти інших ефектів. Наприклад, скорочення високих і/або низьких частот може створити враження звуків, що передаються на радіо.

Ви також можете накладати більше авдіофайлів, змінювати висоту звуку, додавати трохи реверберації або використовувати складніші звукові ефекти, як-от «bit-crusher». Звуковий дизайн може бути цікавим, особливо якщо ви випадково знайдете хороші варіанти звучання вашого авдіо. Експериментування є ключовим, і, можливо, ваш звук стане ще кращим, ніж раніше.

Ми продовжимо лише фільтр EQ, який ми використовували для вирізання проблемної частоти.
...
У Reaper ви можете розділити авдіо, просто перемістивши курсор до місця розрізу та натиснувши <kbd>S</kbd>.
...
Тепер нехай кінцева частина нової доріжки зникне, а початок першої авдіодоріжки з’явиться.

![Цикл із затуханням звукових доріжок](/assets/develop/sounds/dynamic-sounds/step_7.png)

### Експортування {#exporting}

Експортуйте авдіо з двома авдіодоріжками, але лише з одним авдіоканалом (моно) і створіть новий `SoundEvent` для цього `.ogg` файлу у вашому моді.
Якщо ви не впевнені, як це зробити, перегляньте сторінку [створення власних звуків](../sounds/custom).

Тепер це закінчена авдіосистема циклічного відтворення для `SoundEvent` під назвою `ENGINE_LOOP`.
...
Як бачите, нормалізовані значення в поєднанні з лінійною інтерполяцією (lerp) допомагають формувати значення відповідно до бажаних меж авдіо.
...
За допомогою поточної системи ви можете легко обробляти декілька `SoundInstance` одночасно та створювати звук відповідно до ваших потреб.
