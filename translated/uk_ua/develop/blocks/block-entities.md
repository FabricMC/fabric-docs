...
- створити блок, який сам собою реалізує `BlockEntityProvider` і замінити метод `createBlockEntity`
...
Тепер, коли у нас є блок-сутність, ми можемо використовувати його для збереження кількості натискань ПКМ на блоці. Ми зробимо це, додавши поле `clicks` до класу `CounterBlockEntity`:
...
Збереження в NBT здійснюється через `ReadView`s та `WriteView`. Ці подання відповідають за зберігання помилок кодування/декодування та відстеження реєстрів протягом усього процесу серіалізації.

Ви можете читати з `ReadView` за допомогою методу `read`, передаючи `Codec` для потрібного типу. Так само ви можете писати в `WriteView` за допомогою методу `store`, передаючи кодек для типу та значення.

Існують також методи для примітивів, наприклад `getInt`, `getShort`, `getBoolean` тощо для читання та `putInt`, `putShort`, `putBoolean` тощо для запису. Подання також надає методи для роботи зі списками, типами, що допускають значення null, і вкладеними об’єктами.
...
Тут ми додаємо поля, які слід зберегти в переданому `NbtCompound`: у випадку блока лічильника це поле `clicks`.

Читання аналогічно, ви отримуєте значення, які ви зберегли раніше з `ReadView`, і зберігаєте їх у полях BlockEntity:
...
У той час як `writeAdditional` і `loadAdditional` керують збереженням і завантаженням на диск і з диска, все ще є проблема:

- Сервер знає правильне значення «натискань».
- Клієнт не отримує правильне значення під час завантаження блока.
...
Інтерфейс `BlockEntityProvider` також визначає метод під назвою `getTicker`, який можна використовувати для запуску коду кожного тика для кожного екземпляра блока. Ми можемо реалізувати це, створивши статичний метод, який використовуватиметься як `BlockEntityTicker`:
