...
### Додавання вашого блока-предмета до групи предметів {#adding-your-block-s-item-to-an-item-group}

Оскільки `BlockItem` створюється та реєструється автоматично, щоб додати його до групи предметів, ви повинні використати метод `Block.asItem()`, щоб отримати екземпляр `BlockItem`.

У цьому прикладі ми використаємо спеціальну групу предметів, створену на сторінці [власних груп предметів](../items/custom-item-groups).
...
Якщо ви бажаєте, щоб для добування блока був потрібний інструмент, додайте `.requiresTool()` до налаштувань блока, а також додайте відповідний теґ рівня добування.
