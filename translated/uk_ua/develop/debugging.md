...
:::info
Усі режими реєстратора підтримують кілька перевантажень; таким чином ви можете надати більше інформації, наприклад трасування стека!
:::
...
![Консоль показує зареєстровані результати](/assets/develop/debugging/debug_01.png)

У зареєстрованому рядку ви можете знайти:

- `Time` — час, коли було надруковано зареєстровану інформацію
- `Thread` — потік, у якому було надруковано. Ви часто побачите _**серверний потік**_ і _**потік рендера**_; це говорить вам, на якій стороні було виконано код
- `Name` — назва реєстратора. Найкраще використовувати тут свій ID мода, щоб журнали та збої показували, який мод зареєструвався
...
Якщо дані, які ви реєструєте, стосуються лише розробки, може бути корисним створити спеціальний метод `LOGGER` і використовувати його, щоб уникнути друку даних у виробництві.
...
Відсутні ресурси та текстури (фіолетовий і чорний заповнювач) також реєструють попередження на консолі та зазвичай згадують очікувані значення.

![Відсутні ресурси](/assets/develop/debugging/missing_asset.png)

[Вивід реєстартора](/assets/develop/debugging/debug_02.png)
...
| Дія                      | Опис                                                                                                                                                                                                                                               |
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Переступити              | Перехід до наступного виконаного рядка, фактично переміщення екземпляра в логіці                                                                                                                                                                   |
| Крок усередину           | Метод кроку всередину для показу, що відбувається всередині. Якщо в одному рядку є кілька методів, ви можете вибрати, до якого переходити, натиснувши його. Це також необхідно для лямбда-виразів. |
...
:::info
Дії «Переступити» <kbd>F8</kbd> і «Крок усередину» <kbd>F7</kbd> є найпоширенішими, тож спробуйте звикнути до комбінацій клавіш!
:::
...
Ви можете використовувати рядок введення у вікні `Налагодження` для багатьох різних речей, наприклад, ви можете отримати доступ до поточних завантажених об'єктів і використовувати для них методи. Нижче буде додано новий запис із запитаними даними.
...
Усі точки зупину будуть перераховані у вікні `Закладки` IntelliJ.

![Меню точки зупинки](/assets/develop/debugging/debug_09.png)

![Точки зупину в закладках](/assets/develop/debugging/debug_10.png)

### Швидка заміна активного екземпляра {#hotswapping}

Ви можете вносити обмежені зміни в код під час роботи екземпляра, використовуючи дію «Створити > Створити проєкт» зі значком молотка. Ви також можете розмістити значок поруч з елементом спадного меню «Запустити конфігурацію», натиснувши ПКМ порожнє місце у верхній панелі меню IntelliJ.
...
Виняток становлять міксини. Ви можете налаштувати свою конфігурацію запуску, щоб вони також могли змінюватися під час виконання.
...
Користувачі зможуть поділитися цим файлом із розробником мода для подальшої перевірки, як пояснюється в [завантаженнях журналів](../players/troubleshooting/uploading-logs).
...
Усе ще не можете зрозуміти, що відбувається? Ви можете приєднатися до [сервера Fabric Discord](https://discord.com/invite/v6v4pMv) і поспілкуватися зі спільнотою!

Ви також можете переглянути [офіційну вікі Fabric](https://fabricmc.net/wiki/start) для складніших запитів.
