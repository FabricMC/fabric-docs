...
这个指南将会指引您在 Windows 上安装 Java 25。
...
要使用 Minecraft 26.1，你需要安装至少 Java 25。

如果此命令显示的版本低于 25，则需要更新现有的 Java 安装，请继续阅读本页。

:::

## 2. 下载 Java 25 安装程序 {#2-download-the-java-installer}

要安装 Java 25，你需要从 [Adoptium](https://adoptium.net/temurin/releases?version=25&os=windows&arch=any&mode=filter) 下载安装程序。
...
按照安装程序的步骤安装 Java 25。 到达这一页时，应该将以下功能设置为“整个功能将安装在本地硬盘上”：

- `设置或重写 JAVA_HOME 变量`——这会被添加到你的 PATH 中。
- `JavaSoft (Oracle) registry keys`

![高亮了“设置或重写 JAVA_HOME 变量”和“JavaSoft (Oracle) registry keys”的 Java 25 安装器](/assets/players/installing-java/windows-wizard-screenshot.png)
...
## 4. 验证是否已安装 Java 25 {#4-verify-that-java-is-installed}

安装完成后，你可以打开命令提示符并输入 `java -version` 来验证 Java 25 是否已安装。
