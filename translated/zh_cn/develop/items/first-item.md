...
请注意，我们使用了 `GenericItem` 类，它允许我们使用同一个 `register` 方法注册任何继承自 `Item` 类的项。 此外，我们还使用了 [`Function`](https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/util/function/Function.html) 接口作为工厂，它允许我们根据物品属性指定创建物品的方式。
...
:::info
如果想要将物品添加到自定义的 `ItemGroup`，请参阅[自定义创造标签页](./custom-item-groups)页面以了解更多。
:::

这里为举例，我们将这个物品添加到原材料 `ItemGroup` 中，你需要使用 Fabric API 的创造标签页事件——也就是 `ItemGroupEvents.modifyEntriesEvent`。
