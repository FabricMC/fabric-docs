...
`Codec.either` 将两个 codec `Codec<A>` 和 `Codec<B>` 组合为 `Codec<Either<A, B>>`。 产生的 codec 会在反序列化过程中尝试使用第一个 codec，并且*仅当失败时*才尝试使用第二个。
...
由于 JSON 和 NBT 的限制，使用的键的 codec *必须*序列化为字符串。 这包括类型自身不是字符串但会序列化为字符串的 codec，例如 `Identifier.CODEC`。 看看下面的例子：
...
有时，使用*自身*来解码特定字段的 codec 很有用，例如在处理某些递归数据结构时。 在原版代码中，这用于 `Component` 对象，可能会存储其他的 `Component` 作为子对象。 可以使用 `Codec#recursive` 构建这样的 codec。
