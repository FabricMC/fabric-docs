...
以上示例是该任务最基本的用法，它将生成一个 `fabric.mod.json` 文件，其中包含指定的模组 ID 和版本。  该 `json` 代码块支持 [Fabric 模组 JSON 模式](https://fabricmc.net/wiki/documentation:fabric_mod_json)中定义的所有字段。 有关支持的属性的完整列表，请参见 [FabricModJsonV1Spec](https://github.com/FabricMC/fabric-loom/blob/dev/1.12/src/main/java/net/fabricmc/loom/api/fmj/FabricModJsonV1Spec.java)。
