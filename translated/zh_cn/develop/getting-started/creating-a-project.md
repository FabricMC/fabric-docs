...
你可以使用 [Fabric 模板模组生成器](https://fabricmc.net/develop/template/)为你的模组生成一个新项目——你应该填写必要的字段，比如模组名称和包名，以及你想要基于其开发的 Minecraft 版本。

包名应该是小写字母，由点隔开；命名必须唯一，以避免和其他开发者的包冲突。 它通常被格式化为反向网络域名，例如 \`com.example.example-mod。
...
:::info
本网站提供的代码示例使用 [Mojang 的官方名称](../migrating-mappings#mappings)。 如果你的模组使用的映射与这些文档编写的映射不同，你需要使用 [mappings.dev](https://mappings.dev/) 或 [Linkie](https://linkie.shedaniel.dev/mappings?namespace=yarn&translateMode=ns&translateAs=mojang_raw&search=) 等网站转换示例。
:::
...
- 避免使用云存储目录（例如 Microsoft OneDrive）
- 避免使用非 ASCII 字符（例如表情符号、重音字母、汉字）
- 避免使用空格

一个“好”的路径示例可以是：`C:\Projects\YourProjectName`
:::
...
然后应该删除 `.git` 文件夹，并在你的 IDE 中打开项目。 如果找不到 `.git` 文件夹，你需要在你的文件资源管理器中启用显示隐藏文件。
