...
`RenderTickCounter` 类允许检索当前的 `tickProgress` 值。 `tickProgress` 是上一个游戏刻和下一个游戏刻之间的“过程”。

例如，如果我们假设 200 FPS 场景，游戏大约每 10 帧运行一次新的刻。 每一帧，`tickProgress` 代表上一刻与下一刻之间的距离。 超过 11 帧时，你可能会看到：
...
实际上，只有当动画依赖于 Minecraft 刻时，才应该使用 `tickProgress`。 对于基于时间的动画，请使用 `Util.getMillis()`，它可以测量现实世界的时间。

可以调用 `renderTickCounter.getGameTimeDeltaPartialTick(false)` 以检索 `tickProgress`，其中布尔值参数是 `ignoreFreeze`，这实际上只是允许忽略玩家使用 `/tick freeze` 命令的情况。
...
试试使用 `tickProgress` 并查看运行 `/tick freeze` 命令时动画会发生什么情况。 你会看到动画在 `tickProgress` 变为常量时冻结（假设你已将 `false` 作为参数传递给 `DeltaTracker#getGameTimeDeltaPartialTick`）
