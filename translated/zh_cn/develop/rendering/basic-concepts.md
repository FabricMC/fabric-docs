...
`BufferBuilder` 允许我们一个顶点一个顶点地构造我们的缓冲。 要添加顶点，我们使用 `buffer.addVertex(Matrix4f, float, float, float)` 方法。 `Matrix4f` 参数是变换矩阵，我们稍后会详细讨论。 3 个 `float` 参数代表顶点坐标的 (x, y, z)。
...
它通常是通过 `Matrix3x2fStack` 类获得的，而该类可以通过调用 `GuiGraphics#pose()` 方法从 `GuiGraphics` 对象获得。
...
代码展示了 `Matrix3x2fStack`，它自 1.21.8 起用于 HUD 渲染，而下文描述了 `PoseStack`，它的方法略有不同。
