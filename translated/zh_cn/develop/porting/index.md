...
首先将你的模组的 `gradle/wrapper/gradle-wrapper.properties`、`gradle.properties` 和 `build.gradle` 更新到最新版本。 如果遇到问题，可以考虑参考[Fabric 示例模组](https://github.com/FabricMC/fabric-example-mod/tree/26.1)。
...
5. 将 `modImplementation`、`modCompileOnly` 或 `modApi` 的所有实例替换为 `implementation`、`compileOnly` 和 `api`。
