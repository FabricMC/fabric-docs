...
When adding several different varieties of the same block to your block model data generation, it can get quite
repeitive and make it difficult to find everything when you need to make changes.
...
The blocks we will be adding can be shown below:
...
Once you have created your blocks, you begin adding your repetitive generation to the `ModelProvider`.
...
We also need to add the list that contains the colors, the blocks that will be generated, and the tool names of any other tools
blocks:

@[code transcludeWith=:::block-lists](@/reference/latest/src/client/java/com/example/docs/datagen/ExampleModModelProvider.java)

Now we create the loop inside of your `BlockModels` method, this loop will be what actually runs through all of the
repetitive blocks and generates the model JSONs for all of your blocks and their variants.
