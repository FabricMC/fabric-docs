---
title: Loom Remap
description: Documentation for the Fabric Loom Remap Gradle plugin.
authors:
  - Atakku
  - caoimhebyrne
  - Daomephsta
...
Fabric Loom Remap is a variant of Loom used for development of mods for obfuscated versions of Minecraft.
...
Fabric Loom Remap is not recommended for use on Minecraft 26.1 and above, as the additional work done by remapping became unnecessary when the game was deobfuscated.

:::

## Plugin IDs {#plugin-ids}

Fabric Loom Remap uses the following plugin IDs:
...
In addition to the [existing work Loom does to setup a Minecraft workspace](index#setup), Loom Remap does the following:

- Downloads the configured mappings
- Remaps the merged jar with intermediary mappings to produce an intermediary jar
- Remaps the merged jar with your chosen mappings (usually either Yarn or Mojang Mappings) to produce a mapped jar
...
Dependencies declared with `include` are added to `remapJar`. Because of this, jar tasks from other plugins (such as `shadowJar`) will not include these dependencies by default.

:::

## Remapping Configuration {#configuration}

In addition to the [default configuration](index#configuration), extra work is done to setup a remapped workspace.
