...
Fabric API provides a lot of useful hooks and other utilities to perform a variety of tasks in a safe and compatible way, whether that's running code when the server shuts down, registering commands, or rendering to the screen.

Sometimes, however, Fabric API, or the Minecraft code itself, will not provide the tools necessary to accomplish a certain goal. Perhaps you would you like a callback that is invoked whenever an explosion happens, or you would like to modify the logic of mob spawning. In these cases, you will need to directly **modify the bytecode of the game**.
...
## What Makes a Mixin? {#what-makes-a-mixin}
...
Now, consider a mixin that targets this `Person` class:
...
By annotating `PersonMixin` with `@Mixin(Person.class)`, we have turned this regular Java class into a mixin that targets `Person`.

A mixin class behaves differently from regular Java classes. Whenever the game starts (in other words, at runtime), the mixin class will grab all of its members, including fields and methods, and merge them with the mixin's target class.
...
Another thing that mixins allow us to do is declare methods that will have calls to them placed in one or multiple methods of the target class.
...
A common annotation for this would be `@Inject`, that takes as arguments the method that you want to inject a call into, and where in the method to place that call in. This annotation will be used on methods declared in the mixin class.
...
public abstract class PersonMixin {
    ...
    @Inject(method = "getAge", at = @At("HEAD"))
    public void logAgeAccess() {
        System.out.println("The age field of person " + this + " has been accessed!");
    }
}
...
It's important to emphasize the fact that mixins only affect their target classes **at runtime**.
...
What we just did, the process of accessing a member of the target class by marking it as "will exist" in our mixin class, is called **Shadowing**. In the case of methods, the method declaration can be left with an empty body, if the mixin class is non-abstract, or abstract of the mixin class is abstract.
