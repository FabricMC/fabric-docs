...
public class EntityAttributesGameTest implements CustomTestMethodInvoker {
	...
	void testReadingAttributes(GameTestHelper helper, LivingEntity entity) {
		...
		var attribute = entity.getAttribute(ModAttributes.AGGRO_RANGE); // returns an `AttributeInstance`
		...
		// #region reading_entity_attributes
		var value = entity.getAttribute(ModAttributes.AGGRO_RANGE).getValue(); // returns a double with the current value
		var baseValue = entity.getAttribute(ModAttributes.AGGRO_RANGE).getBaseValue(); // returns a double with the base value
		// #endregion reading_entity_attributes
	...
	void testModifyingAttributes(GameTestHelper helper, LivingEntity entity) {
		...
		helper.assertValueEqual(16.0, entity.getAttribute(ModAttributes.AGGRO_RANGE).getValue(), "Expected AGGRO_RANGE attribute to have a value of 16.0 after modifier was added");
		helper.succeed();
	}
