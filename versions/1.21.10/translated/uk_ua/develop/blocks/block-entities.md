...
@[code transcludeWith=:::1](@/reference/1.21.10/src/main/java/com/example/docs/block/entity/custom/CounterBlockEntity.java)

Реєстрація `BlockEntity` дає `BlockEntityType`, подібний до `COUNTER_BLOCK_ENTITY`, який ми використовували вище:

@[code transcludeWith=:::1](@/reference/1.21.10/src/main/java/com/example/docs/block/entity/ModBlockEntities.java)
...
Далі, щоб фактично використовувати блок-сутність нам потрібен блок, який реалізує `EntityBlock`. Нумо створімо один і назвемо його `CounterBlock`.

:::tip
Є два способи підійти до цього:

- створити блок, який розширює `BaseEntityBlock` і реалізувати метод `newBlockEntity`
- створити блок, який сам по собі реалізує `EntityBlock` і замінити метод `newBlockEntity`

У цьому прикладі ми використаємо перший підхід, оскільки `BaseEntityBlock` також надає кілька крутих штук.
:::

@[code transcludeWith=:::1](@/reference/1.21.10/src/main/java/com/example/docs/block/custom/CounterBlock.java)

Використання `BaseEntityBlock` як батьківського класу означає, що нам також потрібно реалізувати метод `codec`, який досить простий.

На відміну від блоків, які є одинарними, нова блок-сутність створюється для кожного екземпляра блоку. Це робиться за допомогою методу `newBlockEntity`, який приймає позицію та `BlockState` і повертає `BlockEntity` або `null`, якщо його не має бути.

Не забудьте зареєструвати блок у класі `ModBlocks`, як у посібнику [Створення вашого першого блоку](../blocks/first-block):

@[code transcludeWith=:::5](@/reference/1.21.10/src/main/java/com/example/docs/block/ModBlocks.java)
...
@[code transcludeWith=:::2](@/reference/1.21.10/src/main/java/com/example/docs/block/entity/custom/CounterBlockEntity.java)
...
@[code transcludeWith=:::2](@/reference/1.21.10/src/main/java/com/example/docs/block/custom/CounterBlock.java)
...
@[code transcludeWith=:::3](@/reference/1.21.10/src/main/java/com/example/docs/block/entity/custom/CounterBlockEntity.java)
...
@[code transcludeWith=:::4](@/reference/1.21.10/src/main/java/com/example/docs/block/entity/custom/CounterBlockEntity.java)
...
У той час як `saveAdditional` і `loadAdditional` керують збереженням і завантаженням на диск і з диска, все ще є проблема:

- Сервер знає правильне значення «натискань».
- Клієнт не отримує правильне значення під час завантаження блоку.

Щоб виправити це, ми замінюємо `getUpdateTag`:

@[code transcludeWith=:::7](@/reference/1.21.10/src/main/java/com/example/docs/block/entity/custom/CounterBlockEntity.java)

Тепер, коли гравець увійде або переміститься в блок, де існує блок, він одразу побачить правильне значення лічильника.

## Тикери {#tickers}

Інтерфейс `EntityBlock` також визначає метод під назвою `getTicker`, який можна використовувати для запуску коду кожного тика для кожного екземпляра блоку. Ми можемо реалізувати це, створивши статичний метод, який використовуватиметься як `BlockEntityTicker`:
...
@[code transcludeWith=:::3](@/reference/1.21.10/src/main/java/com/example/docs/block/custom/CounterBlock.java)
...
@[code transcludeWith=:::5](@/reference/1.21.10/src/main/java/com/example/docs/block/entity/custom/CounterBlockEntity.java)
...
@[code transcludeWith=:::6](@/reference/1.21.10/src/main/java/com/example/docs/block/entity/custom/CounterBlockEntity.java)
