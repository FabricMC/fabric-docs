...
## `TypecInteractionResult` {#typedactionresult}

아이템의 경우, 가장 일반적인 `InteractionResultHolder`는 `ItemStacks`입니다. 이 클래스는 이벤트가 발생한 후 아이템 스택을 교체해야 하는지(또는 교체하지 않아야 하는지) 게임에 알려줍니다.

만약 이벤트에서 아무런 일도 발생하지 않는다면, `InteractionResultHolder#pass(stack)` 메소드에 `stack` 매개 변수로 현재 아이템 스택을 입력해야 합니다.

현재 아이템 스택은 플레이어의 손에서 받아올 수 있습니다. 일반적으로 `InteractionResultHolder`가 필요한 이벤트는 호출 메소드에 손을 전달합니다.
...
`InteractionResult`도 게임에 이벤트의 상태를 알려주는 클래스입니다.
...
:::info
이 이벤트를 사용하는 훌륭한 예시는 [소리 이벤트 재생하기](../sounds/using-sounds)에서 찾을 수 있으며, 플레이어가 블록을 오른쪽 마우스로 클릭했을 때 `useOn` 이벤트에서 소리를 재생합니다.
:::

| 메소드             | 용도                                            |
| --------------- | --------------------------------------------- |
| `hurtEnemy`       | 플레이어가 개체를 공격한 후 실행.           |
| `mineBlock`      | 플레이어가 블록을 채굴한 후 실행.           |
| `inventoryTick` | 아이템이 보관함에 있을 때 매 틱 마다 실행.     |
| `onCraftedPostProcess`       | 아이템이 제작되었을 때 실행.              |
| `useOn`    | 아이템을 들고 블록에 오른쪽 마우스로 클릭하면 실행. |
