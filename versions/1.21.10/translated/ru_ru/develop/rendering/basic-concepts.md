...
Хотя большая часть рендеринга в Minecraft абстрагирована с помощью различных методов `GuiGraphics`, и вам, скорее всего, не придется касаться ничего из упомянутого здесь, все равно важно понимать основы того, как работает рендеринг.

## `Tesselator` {#the-tessellator}

`Tesselator` — это основной класс, используемый для рендеринга объектов в Minecraft. Это singleton, то есть в игре существует только один его экземпляр. Экземпляр можно получить с помощью `Tesselator.getInstance()`.
...
`Tesselator` используется для создания `BufferBuilder`, который используется для форматирования и загрузки данных рендеринга в OpenGL.

### Инициализация `BufferBuilder` {#initializing-the-bufferbuilder}

Прежде чем что-либо записать в `BufferBuilder`, его необходимо инициализировать. Это делается с помощью метода `Tesselator#begin(...)`, который принимает `VertexFormat` и режим отрисовки и возвращает `BufferBuilder`.
...
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Mode.LINES`            | Каждый элемент состоит из 2 вершин и представлен в виде одной линии.                                                                                         |
| `Mode.LINE_STRIP`       | Для первого элемента требуется 2 вершины. Дополнительные элементы рисуются всего с одной новой вершиной, создавая непрерывную линию.         |
| `Mode.DEBUG_LINES`      | Аналогично `Mode.LINES`, но линия на экране всегда имеет ширину ровно в один пиксель.                                                                    |
| `Mode.DEBUG_LINE_STRIP` | То же, что и `Mode.LINE_STRIP`, но линии всегда имеют ширину в один пиксель.                                                                             |
| `Mode.TRIANGLES`        | Каждый элемент состоит из 3 вершин, образующих треугольник.                                                                                                  |
| `Mode.TRIANGLE_STRIP`   | Начинается с 3 вершин первого треугольника. Каждая дополнительная вершина образует новый треугольник с двумя последними вершинами.           |
| `Mode.TRIANGLE_FAN`     | Начинается с 3 вершин первого треугольника. Каждая дополнительная вершина образует новый треугольник с первой вершиной и последней вершиной. |
| `Mode.QUADS`            | Каждый элемент состоит из 4 вершин, образующих четырехугольник.                                                                                              |

### Запись в `BufferBuilder` {#writing-to-the-bufferbuilder}

После инициализации `BufferBuilder` вы можете записывать в него данные.

`BufferBuilder` позволяет нам создавать наш буфер, вершина за вершиной. Чтобы добавить вершину, мы используем метод `buffer.addVertex(matrix, float, float, float)`. Параметр `matrix` — это матрица преобразования, которую мы рассмотрим более подробно позже. Три параметра с плавающей точкой представляют собой координаты (x, y, z) положения вершины.
...
Обычно его можно получить через класс `PoseStack`, который можно получить через объект `GuiGraphics`:
...
Проще объяснить, как писать в `BufferBuilder`, на практическом примере. Допустим, мы хотим что-то визуализировать, используя режим рисования `Mode.TRIANGLE_STRIP` и формат вершин `POSITION_COLOR`.
...
@[code lang=java transcludeWith=:::1](@/reference/1.21.10/src/client/java/com/example/docs/rendering/RenderingConceptsEntrypoint.java)
...
## `PoseStack` {#the-matrixstack}

Узнав, как писать в `BufferBuilder`, вы, возможно, зададитесь вопросом, как преобразовать свою модель или даже анимировать ее. Вот тут-то и появляется класс `PoseStack`.

Класс `PoseStack` имеет следующие методы:
...
Взяв за основу приведенный выше пример, мы можем масштабировать наш ромб вверх и вниз, используя `PoseStack` и `tickDelta`, которые представляют собой "прогресс" между последним игровым тиком и следующим игровым тактом. Мы разъясним это позже на странице [Рендеринг в Hud](./hud#tick-delta).
...
@[code lang=java transcludeWith=:::2](@/reference/1.21.10/src/client/java/com/example/docs/rendering/RenderingConceptsEntrypoint.java)
...
Кватернионы — это способ представления вращений в трехмерном пространстве. Они используются для поворота верхней матрицы в `PoseStack` с помощью метода `mulPose(Quaternion, x, y, z)`.
...
Допустим, мы хотим повернуть наш алмаз вокруг оси z. Мы можем сделать это с помощью `PoseStack` и метода `mulPose(Quaternion, x, y, z)`.

@[code lang=java transcludeWith=:::3](@/reference/1.21.10/src/client/java/com/example/docs/rendering/RenderingConceptsEntrypoint.java)
