...
Подобно предметам, блоки имеют в своем конструкторе класс `BlockBehaviour.Properties`, который определяет такие свойства блока как его звуковые эффекты и уровень добычи.
...
- Нам нужен `ResourceKey<Block>`, который используется как уникальный идентификатор нашего блока, он передается в Registry.register в предыдущем методе утилиты.
- `ResourceKey<Block>` также требуется сборщику `BlockBehaviour.Properties`.

:::tip
Вы можете использовать `BlockBehaviour.Properties.ofFullCopy(AbstractBlock block)`, чтобы скопировать параметры существующего блока. В данном случае мы могли бы использовать `Blocks.DIRT` для копирования параметров блока земли, но для примера мы используем конструктор.
:::
