...
You can retrieve `gameTimeDeltaPartialTick` by calling `renderTickCounter.getGameTimeDeltaPartialTick(false)`, where the boolean parameter is `ignoreFreeze`, which essentially just allows you to ignore whenever players use the `/tick freeze` command.
