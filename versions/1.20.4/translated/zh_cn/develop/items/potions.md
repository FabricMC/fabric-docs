...
添加药水和添加物品的方式类似。 你将创建的你的药水的一个实例，并通过调用 `PotionBrewing.addMix` 注册它。

:::info
当 Fabric API 存在时，`PotionBrewing.addMix` 的访问权限会被访问加宽器（Access Widener）设置为 `public` 。
:::
...
- `MobEffect type` - 一个状态效果。 我们在这里使用我们的自定义效果。 此外你也可以通过 `net.minecraft.entity.effect.MobEffects` 访问原版效果。
...
在我们的入口点中，我们调用 `PotionBrewing.addMix`。
...
如果你使用 Fabric API，就没有必要使用 Mixin 注入 `@Invoker`，可以直接调用 `PotionBrewing.addMix`。
...
net.fabricmc.fabric.api.registry.FabricPotionBrewing` 可以使用 `Ingredient` 而非 `Item` 来注册药水配方。
:::

### 不使用 Fabric API 注册药水配方

没有 Fabric API 时，`PotionBrewing.addMix` 将会是 `private` 的。 为了访问这个方法，请使用如下的 `@Invoker` Mixin，或使用访问加宽器（Access Widener）。
