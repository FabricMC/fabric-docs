...
尽管 Minecraft 的大部分渲染都通过 `GuiGraphics` 中的多种方法被抽象出来，且您很可能并不需要接触这里提到的任何内容，了解渲染的基础实现依然是重要的。
...
镶嵌器 `Tesselator` 负责创建一个缓冲构建器 `BufferBuilder`，用于将渲染数据格式化并上传到 OpenGL。 您可以通过 `Tesselator.getBuffer()` 来创建一个缓冲构建器 `BufferBuilder`。
...
它通常通过 `PoseStack` 类获取，而 `PoseStack` 实例可以从 `GuiGraphics` 对象获取：
...
从我们上面的案例出发，我们可以用 `PoseStack` 和 `tickDelta`（从上一帧到现在经过的时间）让我们的菱形放大和缩小。
...
四元数是三维空间中的旋转的一种表示。 通过 `mulPose(Quaternion, x, y, z)` 方法，它们被用于旋转 `PoseStack` 栈顶的矩阵。
