...
- `Renderable` 用于指定渲染逻辑。当一个组件实现此接口时，您可以通过 `Screen#addDrawableChild` 将组件对象添加至界面中。
- `GuiEventListener` 用于接收用户输入事件，比如鼠标点击、键盘输入等。
- `NarrationSupplier` 用于提供无障碍信息，无障碍功能（如复述）通过此接口访问组件内容。
- `NarratableEntry` 用于聚焦组件，实现此接口后组件可以由 `Tab` 键选中，这也是一种无障碍功能。
