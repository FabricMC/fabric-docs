...
Das Hinzufügen eines Trankes ist vergleichbar mit dem Hinzufügen eines Items. Erstelle eine Instanz des Trankes und registriere ihn mit `PotionBrewing.addMix`.

:::info
Falls die Fabric API vorhanden ist, kann auf `PotionBrewing.addMix` durch einen "Access Widener" zugegriffen werden.
:::
...
- `MobEffect type` - Ein Effekttyp. Hier verwenden wir unseren benutzerdefinierten Effekt. Alternativ kannst du auf Vanilla-Effekte über `net.minecraft.entity.effect.MobEffects` zugreifen.
...
In unsere Klasse, die für das Initialisieren zuständig ist, wird dann die `PotionBrewing.addMix` Methode aufgerufen.
...
Wenn die Fabric API benutzt wird, wird der Mixin Invoker nicht benötigt und die `PotionBrewing.addMix` Methode kann direkt aufgerufen werden.
...
net.fabricmc.fabric.api.registry.FabricPotionBrewing` Methode, benutzt werden.
:::

### Registrieren des Tranks ohne der Fabric API

Ohne de Fabric API ist die Methode `PotionBrewing.addMix` privat. Um auf diese Methode zuzugreifen, verwende den folgenden Mixin Invoker oder verwende einen "Access Widener".
