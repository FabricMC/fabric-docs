...
DataResult<JsonElement> result = mapCodec.encodeStart(JsonOps.INSTANCE, Map.of(
    new ResourceLocation("example", "nombre"), 23,
    new ResourceLocation("example", "le_nombre_plus_cool"), 42
));
```
...
Il est parfois utile d'avoir un codec qui s'utilise _soi-même_ pour décoder certains champs, par exemple avec certaines structures de données récursives. Le code vanilla en fait usage pour les objets `Component`, qui peuvent stocker d'autres `Component`s en tant qu'enfants. Un tel codec peut être construit grâce à `Codecs#createRecursive`.
