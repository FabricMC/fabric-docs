...
Prima di poter scrivere al `BufferBuilder`, devi inizializzarlo. Questo viene fatto usando `Tesselator#begin(...)`, che prende un `VertexFormat` e una modalità di disegno e restituisce un `BufferBuilder`.
...
Usando l'esempio di prima, possiamo ingrandire e rimpicciolire il nostro diamante usando la `PoseStack` e il `tickDelta` - che è il tempo passato dall'ultimo frame.
