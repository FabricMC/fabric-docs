...
Es gibt mehrere Möglichkeiten, eine Widget-Klasse zu erstellen, beispielsweise durch die Erweiterung von `AbstractWidget`. Diese Klasse bietet viele nützliche Funktionen, wie beispielsweise die Verwaltung von Breite, Höhe und Position sowie die Behandlung von Events.

- `Renderable` - zum Rendern - Erforderlich, um das Widget über die Methode `addRenderableWidget` in der Oberfläche zu registrieren.
...
Wie alle Widgets musst du es mit der Methode `addRenderableWidget`, die von der Klasse `Screen` bereitgestellt wird, zur Oberfläche hinzufügen. Stelle sicher, dass du dies in der Methode `init` machst.
...
Du kannst Events wie Mausklicks und Tastendrücke behandeln, indem du die Methoden `mouseClicked`, `afterMouseAction`, `keyPressed` und andere Methoden überschreibst.
