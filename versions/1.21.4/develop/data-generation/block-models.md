...
What does `TextureMap` do? It actually provides the ResourceLocations that point to the textures. It technically behaves like a normal map - you associate a `TextureKey` (Key) with a `ResourceLocation` (Value).
