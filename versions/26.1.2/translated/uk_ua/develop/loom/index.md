...
authors:
  ...
  - SolidBlock-cn
resources:
  https://github.com/fabricMC/fabric-loom: Початковий код Fabric Loom
---
...
Якщо ви використовуєте розділені початкові набори в збірці з кількома проєктами, вам також потрібно буде додати залежність для клієнтського початкового набору іншого проєкту.
...
Протягом багатьох років поширеним джерелом збоїв на сервері були моди, які випадково викликали клієнтський код під час встановлення на сервері. Новіші версії Loom і Завантажувача надають можливість вимагати переміщення всього клієнтського коду до власного початкового набору. Це робиться для того, щоб розв'язати проблему під час компіляції, але збірка все одно призведе до єдиного файлу jar, який працює з обох сторін.

Наступний фрагмент із файлу `build.gradle` показує, як можна ввімкнути це для свого мода. Оскільки тепер ваш мод буде розділено на два набори джерел, вам потрібно буде використовувати новий DSL, щоб визначити набори джерел вашого мода. Це дозволяє Завантажувачу Fabric згрупувати шлях до класів вашого мода. Це також корисно для деяких інших складних налаштувань кількох проєктів.
...
- `include`: оголошує залежність, яку слід включити як jar усередині jar у кінцевий вихід мода. Це налаштування залежності не є транзитивним. Для залежностей, не пов’язаних із модами, Loom згенерує файл jar мода з `fabric.mod.json`, використовуючи ID мода як назву та ту саму версію

::: info

Залежності, оголошені за допомогою `include`, додаються до `remapJar` у перемапованому Loom або до `jar` у неперемапованому Loom відповідно. Через це завдання jar з інших плаґінів (таких як `shadowJar`) не включатимуть ці залежності усталено.
