---
title: 移植到 Fabric API 26.1
description: 将你的模组更新到 Minecraft 26.1 的 Fabric API 的指南。
authors:
  - modmuss50
resources:
  https://www.jetbrains.com/help/idea/migrate.html: 迁移重构 - IntelliJ IDEA 文档
---

由于 Minecraft 26.1 现在不再混淆，大多数开发者现在将使用 Mojang 的官方映射。 之前，Fabric API 是基于 Yarn 映射构建的，但随着迁移到官方映射，API 名称已更新以匹配适用的官方名称。 这些更改不向后兼容，因此您需要更新您的模组以使用新名称。 这些 API 重命名不会由迁移映射工具处理，如果您的模组是基于 Yarn 映射构建的，强烈建议首先遵循[迁移映射](../../../develop/porting/mappings/)的说明。

以下是 Fabric API 26.1 中所有重命名类、方法和字段的列表，使用此列表的最佳方法是在此页面上搜索(<kbd>⌘/CTRL</kbd>+<kbd>F</kbd>)旧名称并将其替换为新名称。
