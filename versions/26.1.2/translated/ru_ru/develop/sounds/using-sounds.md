...
В этом примере методы `interactLivingEntity()` и `useOn()` для пользовательского интерактивного предмета используются для воспроизведения звука «Размещение медного блока» и звука мародёра.

<<< @/reference/26.1.2/src/main/java/com/example/docs/item/custom/CustomSoundItem.java#interact

Метод`playSound()` используется вместе с объектом `LivingEntity`. Необходимо указать только SoundEvent, громкость и высоту тона. Вы также можете использовать метод `playSound()` из экземпляра мира (level), чтобы получить более высокий уровень контроля.
