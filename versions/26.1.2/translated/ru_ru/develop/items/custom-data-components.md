...
authors:
  ...
  - ekulxam
resources:
  https://minecraft.wiki/w/Data_component_format#List_of_components: Список ванильных компонентнов - Minecraft Wiki
---
...
Как и в случае со всем остальным в вашем моде, вам нужно будет зарегистрировать свой собственный компонент с помощью `DataComponentType`. Этот тип компонента принимает универсальный аргумент, содержащий тип значения вашего компонента. Мы более подробно рассмотрим это далее, когда будем рассматривать [базовые](#basic-data-components) и [расширенные](#advanced-data-components) компоненты.
...
<<< @/reference/26.1.2/src/main/java/com/example/docs/component/ModComponents.java#mod_components
...
И наконец, у нас есть `DataComponentType.Builder`, который создаёт фактический экземпляр `DataComponentType`, подлежащий регистрации. Здесь содержится еще одна важная деталь, которую нам нужно будет обсудить: `Codec` вашего компонента. В настоящее время это поле пустое, но мы скоро его заполним.
...
<<< @/reference/26.1.2/src/main/java/com/example/docs/component/ModComponents.java#integer_component

Вы можете видеть, что теперь мы передаем `<Integer>` в качестве нашего универсального типа, указывая, что этот компонент будет сохранен как одно значение `int`. В качестве нашего кодера мы используем предоставленный кодек `ExtraCodecs.POSITIVE_INT`. Для таких простых компонентов, как этот, можно обойтись использованием базовых кодеков, но для более сложных сценариев может потребоваться специальный кодек (об этом мы кратко поговорим позже).
...
<<< @/reference/26.1.2/src/main/java/com/example/docs/item/custom/CounterItem.java#item
...
Это вернет текущее значение компонента как тип, который мы определили при регистрации нашего компонента. Затем мы можем использовать это значение для добавления записи всплывающей подсказки.

### Добавление всплывающих подсказок {#append-tooltips}

Для простоты мы добавим эту строку в метод `appendHoverText` в классе `CounterItem`. Обратите внимание, что этот метод помечен как устаревший, поскольку Mojang работает над тем, чтобы поведение предметов обрабатывалось исключительно через компоненты, а не через сами предметы.
...
Начиная с версии 1.21.5, метод `appendHoverText` помечен как устаревший. Теперь рекомендуется реализовывать `TooltipProvider` следующим образом. Для этого потребуется [создание собственного класса компонента](#advanced-data-components).

<<< @/reference/26.1.2/src/main/java/com/example/docs/component/ComponentWithTooltip.java#component_with_tooltip

Затем вы можете зарегистрировать `TooltipProvider` через `ItemComponentTooltipProviderRegistry`. Это вызывается в `onInitialize` в `ModInitializer`.

<<< @/reference/26.1.2/src/main/java/com/example/docs/ExampleMod.java#tooltip_provider

В качестве альтернативы вы можете использовать `ItemTooltipCallback` вместо `appendHoverText`. Это вызывается в `onInitializeClient` в `ClientModInitializer`.

<<< @/reference/26.1.2/src/client/java/com/example/docs/ExampleModClient.java#tooltip_provider_client
...
Когда вы регистрируете свой предмет и передаёте объект `Item.Properties` в его конструктор, вы также можете указать список компонентов по умолчанию, которые будут применяться ко всем новым предметам. Если вернуться к нашему классу `ModItems`, где мы регистрируем `CounterItem`, мы можем добавить значение по умолчанию для нашего пользовательского компонента. Добавьте это, чтобы для новых элементов отображалось количество «0».

<<< @/reference/26.1.2/src/main/java/com/example/docs/item/ModItems.java#counter
...
Вы также можете проверить наличие конкретного компонента в `ItemStack` с помощью метода `has()`. Он принимает тип компонента в качестве аргумента и возвращает `true` или `false` в зависимости от того, содержит ли стек этот компонент.
...
<<< @/reference/26.1.2/src/main/java/com/example/docs/item/custom/CounterItem.java#fixed_append_hover_text
...
<<< @/reference/26.1.2/src/main/java/com/example/docs/item/custom/CounterItem.java#use
...
<<< @/reference/26.1.2/src/main/java/com/example/docs/component/ModComponents.java#custom_component
...
Вы также можете задать значение по умолчанию для составного компонента, передав объект компонента в ваши `Item.Properties`. Например:
...
### Добавление продвинутых всплывающих подсказок {#advanced-tooltips}

Поскольку у нас есть собственный класс компонента, мы можем реализовать `TooltipProvider` следующим образом и избежать устаревшего метода `appendHoverText`

<<< @/reference/26.1.2/src/main/java/com/example/docs/component/AdvancedCustomComponent.java#advanced_custom_component

Затем вы можете зарегистрировать `TooltipProvider` через `ItemComponentTooltipProviderRegistry`. Это вызывается в `onInitialize` в `ModInitializer`.

<<< @/reference/26.1.2/src/main/java/com/example/docs/ExampleMod.java#advanced_tooltip_provider
