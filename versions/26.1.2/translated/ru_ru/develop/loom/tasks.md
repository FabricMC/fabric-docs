...
Приведённый пример — это базовое использование задачи. Он создаст файл `fabric.mod.json` с указанными `modId` и `version`. Блок `json` поддерживает все поля, определённые в [схеме Fabric Mod JSON](../loader/fabric-mod-json).
