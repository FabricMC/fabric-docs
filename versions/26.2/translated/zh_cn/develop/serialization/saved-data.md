...
<<< @/reference/26.2/src/main/java/com/example/docs/Unreferenced.java#saved_data_example_scenario
...
<<< @/reference/26.2/src/main/java/com/example/docs/saveddata/SavedBlockData.java#basic_structure

为了序列化和反序列化这些数据，我们需要定义一个 Codec。 我们可以使用 Minecraft 提供的各种基础 Codec 来组合构建一个 Codec。

你需要一个带 `int` 参数的构造方法来初始化这个类。

<<< @/reference/26.2/src/main/java/com/example/docs/saveddata/SavedBlockData.java#ctor

然后我们可以构建一个 Codec。

<<< @/reference/26.2/src/main/java/com/example/docs/saveddata/SavedBlockData.java#codec

我们应该在数据实际修改时调用 `setDirty()`，以便 Minecraft 知道应该将其保存到磁盘。

<<< @/reference/26.2/src/main/java/com/example/docs/saveddata/SavedBlockData.java#set_dirty

最后，我们需要有一个 `SavedDataType` 来描述我们的保存数据。 第一个参数对应将在世界的 `data` 目录中创建的文件名。

<<< @/reference/26.2/src/main/java/com/example/docs/saveddata/SavedBlockData.java#type
...
<<< @/reference/26.2/src/main/java/com/example/docs/saveddata/SavedBlockData.java#method
...
<<< @/reference/26.2/src/main/java/com/example/docs/saveddata/ExampleModSavedData.java#event_registration
