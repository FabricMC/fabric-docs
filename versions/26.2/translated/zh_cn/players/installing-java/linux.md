---
title: 在 Linux 上安装 Java
description: 在 Linux 上安装 Java 的逐步指南。
authors:
  - IMB11
next: false
---

这个指南将会指引您在 Linux 上安装 Java 25。

Minecraft 启动器自带 Java 安装，因此本节仅在你想要使用 Fabric 的 `.jar` 安装程序，或想要使用 Minecraft 服务器 `.jar` 时才相关。

## 1. 检查 Java 是否已经安装{#1-check-if-java-is-already-installed}

打开终端输入 `java -version` 并按下 `回车`。

![输入 "java -version" 的终端](/assets/players/installing-java/linux-java-version.png)

::: warning

要使用 Minecraft 26.1，你需要安装至少 Java 25。

如果此命令显示的版本低于 25，则需要更新现有的 Java 安装，请继续阅读本页。

:::

## 2. 下载并安装 Java 25 {#2-downloading-and-installing-java}

我们推荐使用 OpenJDK 25，可以在大多数 Linux 发行版中可用。

### Arch Linux {#arch-linux}

::: info

更多在 Arch Linux 上安装 Java 的信息可以参考 [Arch Linux 中文维基](https://wiki.archlinuxcn.org/wiki/Java)。

:::

您可以从官方仓库安装最新版 JRE：

```sh
sudo pacman -S jre-openjdk
```

若您运行的服务器无需图形用户界面，可以安装无头版本：

```sh
sudo pacman -S jre-openjdk-headless
```

如果您计划开发模组，您需要安装 JDK：

```sh
sudo pacman -S jdk-openjdk
```

### Debian/Ubuntu{#debian-ubuntu}

您可以用以下命令使用 `apt` 安装 Java 25：

```sh
sudo apt update
sudo apt install openjdk-25-jdk
```

### Fedora{#fedora}

您可以用以下命令使用 `dnf` 安装 Java 25：

```sh
sudo dnf install java-25-openjdk
```

如果不需要图形化界面，可以安装 headless 版本：

```sh
sudo dnf install java-25-openjdk-headless
```

如果您计划开发模组，您需要安装 JDK：

```sh
sudo dnf install java-25-openjdk-devel
```

### 其他 Linux 发行版{#other-linux-distributions}

如果您的发行版未在上文列出，您可以从 [Adoptium](https://adoptium.net/installation/linux) 下载最新版 JRE

若您计划开发模组，请参阅针对您发行版的其它指南。

## 3. 验证是否已安装 Java 25 {#3-verify-that-java-is-installed}

安装完成后，您可以打开终端并输入 `java -version` 来验证 Java 25 是否已安装。

若命令执行成功，你将看到与前文类似的界面，其中会显示 Java 版本：

![输入 "java -version" 的终端](/assets/players/installing-java/linux-java-version.png)
