...
Щоб отримати доступ до полів або методів, безпечніше та простіше використовувати [міксини доступу](../mixins/accessors), але є дві ситуації, коли засобів доступу недостатньо і необхідне розширення доступу:
...
Записи розширювача доступу починаються з одного з трьох базових ключових слів директиви, які вказують тип модифікації, яку слід застосувати.

Після ключового слова йдуть параметри — зазвичай цілі розширення.

На один і той самий клас, метод або поле може поширюватися дія кількох записів про розширення доступу — по одному в кожному рядку.

Директиви доступу також можна зробити [транзитивними](../class-tweakers/index#transitive-entries), додавши префікс `transitive-` перед базовою директивою доступу.
...
`extendable` можна застосовувати лише до класів і методів:

- Класи стають публічними та некінцевими.
- Методи `non-final`, а також стають `protected`, якщо спочатку вони були `private`.
...
<<< @/reference/26.2/src/main/resources/example-mod.classtweaker#accesswidening_examples_classes
...
<<< @/reference/26.2/src/main/resources/example-mod.classtweaker#accesswidening_examples_methods
...
<<< @/reference/26.2/src/main/resources/example-mod.classtweaker#accesswidening_examples_fields
...
[mcsrc](https://mcsrc.dev) дозволяє декомпілювати початковий код Minecraft і переглядати його безпосередньо в браузері, а також копіювати в буфер обміну цілі для міксина, розширювача доступу або перетворювачів доступу.
...
Щоб побачити внесені зміни, оновіть проєкт Gradle з [перегенеруванням джерел](../getting-started/generating-sources). Необхідно відповідним чином змінити обмеження доступу для елементів, які ви націлили. Обов'язково знову відкрийте цільовий клас із декомпільованого початкового коду, щоб побачити внесені зміни.

::: tip

Якщо зміни не показано, ви можете спробувати [перевірити файл](../class-tweakers/index#validating-the-file) та перевірити на наявність помилок.
