...
Використання «чистого» OpenGL у [Minecraft 26.2 не підтримується, оскільки ця версія вийшла з необов'язковим бекендом Vulkan](https://www.minecraft.net/en-us/article/another-step-towards-vibrant-visuals-for-java-edition). Натомість вам слід використовувати рівень абстракції `Blaze3D`, який розташовується між вашим кодом і бекендом рендера (OpenGL або Vulkan).
...
`BufferBuilder` — це клас, що використовується для форматування та завантаження даних рендера до бекенду. Він використовується для створення буфера, який потім передається на бекенд для малювання.

### Формати вершин {#vertex-formats}

`VertexFormat` визначає елементи, які ми включаємо до буфера даних, і описує, як ці елементи мають передаватися до бекенду.
...
Цей метод повертає конструктор вершин, за допомогою якого ми можемо вказати додаткову інформацію для вершини. Під час додавання цієї інформації вкрай важливо дотримуватися порядку визначеного нами `VertexFormat`. Якщо ми цього не зробимо, бекенд може неправильно інтерпретувати наші дані. Після того, як ми закінчили побудову вершини, просто продовжуйте додавати інші вершини та дані до буфера, доки не закінчите.
