---
title: Байт-код Java
description: Дізнайтеся про байт код Java та як можна писати міксини ефективніше.
authors:
  - Earthcomputer
  - its-miroma
  - Kilip1000
resources:
  https://en.wikipedia.org/wiki/List_of_JVM_bytecode_instructions: Список інструкцій байт-коду JVM – Вікіпедія
---
...
Багато речей у байт-коді, як-от класи, поля та методи, все ще ідентифікуються за іменем (і дескриптором для полів і методів, ми дійдемо до цього), так само як і в початковому коді. Однак точний формат цих назв дещо відрізняється.
...
З іншого боку, статичні ініціалізатори (блок `static {}` у початковому коді, а також ініціалізатори статичних полів за деякими винятками) також є ще одним методом, який запускається під час завантаження класу: його ім’я `<clinit>`, а його дескриптор `()V`.

## Локальні змінні {#local-variables}

У початковому коді локальні змінні ідентифікуються за їх назвою. У байт-коді вони натомість ідентифікуються числом або індексом у таблиці локальних змінних (LVT, ТЛЗ). Параметри методу включені до ТЛЗ, як і об’єкт `this` в нестатичних методах.
...
Це стосується Minecraft 26.1 і новіших версій, оскільки ці версії не обфусковані. Зауважте, що [попередні версії Minecraft обфусковані](../porting/mappings/#whats-going-on-with-mappings).
...
Завантажує змінну зі слота ТЛЗ 0 (`this`) і надсилає її значення до стека операндів.
...
Витягує верхнє значення зі стека операндів, отримує значення свого поля `x` (яке, як ми сказали, було 42), і надсилає його до стека операндів.
...
Завантажує змінну зі слота ТЛЗ 1 (`offset`) і надсилає її значення до стека операндів.
...
Витягує два верхніх значення зі стека операндів, додає їх і надсилає цю суму до стека операндів.
...
Витягує верхнє значення зі стека операндів і призначає його локальній змінній у слоті ТЛЗ 2 (`result'` який є [локальною змінною](#variable-instructions)).
...
Завантажує змінну зі слота ТЛЗ 2 (`result`) і надсилає її значення до стека операндів.
...
Ось посилання на найпоширеніші інструкції та шаблони байт-коду, з якими ви зіткнетеся під час розробки міксинів. Щоб отримати повний розширений список інструкцій, перегляньте [Список інструкцій щодо байт-коду JVM на Вікіпедії](https://en.wikipedia.org/wiki/List_of_JVM_bytecode_instructions).
...
Інструкції завантаження зчитують [значення з ЛЗТ](#local-variables) і надсилають його до стека операндів.
...
У початковому коді написання `new MyClass()` створює новий екземпляр `MyClass` і викликає його конструктор. У байт-коді ці два кроки стають різними операціями. Візьмемо, наприклад, такий код:
...
Завантажує змінну зі слота ТЛЗ 0 (`level`) і надсилає її значення до стека операндів.
...
Витягує параметри, які буде подано в `Creeper.<init>` (у цьому випадку `level`), і показник на об’єкт, який буде ініціалізовано, зі стека операндів, а потім викликає конструктор.
