...
authors:
  ...
  - skippyall
resources:
  https://docs.neoforged.net/docs/resources/server/recipes/custom/: Benutzerdefinierte Rezepte - NeoForge Docs
  https://docs.neoforged.net/docs/resources/server/recipes/ingredients/: Zutaten - NeoForge Docs
---
...
Wenn du versuchst, einen benutzerdefinierten Rezepttyp für einen Vanilla-Arbeitsplatz hinzuzufügen, lies bitte den Abschnitt [Vanilla-Rezepte erweitern](./extending-vanilla-recipes).

:::

Benutzerdefinierte Rezepttypen bieten die Möglichkeit, datengetriebene Rezepte für die benutzerdefinierten Herstellungsmechaniken deines Mods zu erstellen. Als Beispiel erstellen wir einen Rezepttyp für einen Aufwertungsblock, ähnlich wie bei einem Schmiedetisch.
...
Alternativ könnten wir die beiden `ItemStack`s durch eine `List` ersetzen. Dies gilt auch für die anderen Klassen, die die beiden `Ingredient`- oder `ItemStack`-Objekte verwenden.
...
Beachte, dass wir für unsere Eingaben `Ingredient`-Objekte verwenden. Dadurch kann unser Rezept mehrere Items beliebig austauschbar verarbeiten. (Siehe auch: Zutaten-Seite aus der NeoForge-Dokumentation im Abschnitt Ressourcen.)
...
Der Rezept-Serializer verwendet einen [`MapCodec`](../serialization/codecs/#mapcodec), um das Rezept aus JSON zu lesen, und einen `StreamCodec`, um es über das Netzwerk zu senden.
...
Möchtest du einen benutzerdefinierten Arbeitsplatz erstellen, der zu deinem Rezepttyp passt? Siehe [Arbeitsplätze](../blocks/workstations).
